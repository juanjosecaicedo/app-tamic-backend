...
function getServices(req, res){
    db.sequelize.query("SELECT s.service_id, CONCAT(p.name,' ', p.lastname) AS name, pl.name AS plan_name, pl.description AS plan_description FROM tbl_services s, tbl_users u, tbl_people p, tbl_plans pl WHERE s.tbl_plans_plan_id = pl.plan_id AND s.tbl_users_user_id = u.user_id AND u.user_id = p.tbl_users_user_id", {
        type: db.sequelize.QueryTypes.SELECT
    })
    .then(servies => {
        res.send(servies);
